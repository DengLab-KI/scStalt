staltIntegration <- function(count1,count2,nPoints=100,ref) {
    ...
    b1_id$cosine_similarity_score = as.numeric(b1_id$cosine_similarity_score)
    b1_id$stage_assigned= as.numeric(b1_id$stage_assigned)
    b2 = count2[genes,]
    b2 = as.data.frame(b2)
   ...
   b2_c = as.data.frame(b2@assays$RNA@counts)
    for (i in 1:nPoints){
        ...
        s2[is.na(s2)]=0
        s2[is.infinite(s2)]=0
         b2_c[,which(b2_id$stage_assigned==i)] =
